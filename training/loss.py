    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                x_0 = torch.cat([gen_img,gen_img],0)
                ...
                gen_logits = self.run_D(gen_img, gen_c,t, sync=False)
                loss_Gmain = torch.nn.functional.mse_loss(gen_logits, noise, reduction='none').sum()/2# / 1000    
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                t = torch.randint(T, size=(x_0.shape[0], ), device=x_0.device)  
                noise = torch.randn_like(x_0, device=x_0.device)                
                noise2 = torch.randn_like(x_0, device=x_0.device)                
                #noise2 = torch.zeros_like(x_0, device=x_0.device)                
                gen_img = (
                    x_0 +
                    extract(sqrt_one_minus_alphas_bar, t, x_0.shape) * noise)
                gen_logits = self.run_D(gen_img, gen_c,t, sync=False)
                loss_Dgen =  torch.nn.functional.mse_loss(gen_logits, noise2, reduction='none').sum() #/ 1000   
                
                #gen_logits = self.run_D(gen_img, gen_c, sync=False) # Gets synced by loss_Dreal.
                training_stats.report('Loss/scores/fake', loss_Dgen)
                #training_stats.report('Loss/signs/fake', gen_logits.sign())
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_img_tmp = (
                                x_0 +
                                extract(sqrt_one_minus_alphas_bar, t, x_0.shape) * noise)
                real_logits = self.run_D(real_img_tmp, real_c,t, sync=sync)
            
                
                #real_logits = self.run_D(real_img_tmp, real_c, sync=sync)
                #training_stats.report('Loss/scores/real', real_logits)
                training_stats.report('Loss/signs/real', (1-real_logits.mean()).sign())

                loss_Dreal = 0
                if do_Dmain:
                    loss_Dreal = torch.nn.functional.mse_loss(real_logits, noise, reduction='none').sum()# / 1000
